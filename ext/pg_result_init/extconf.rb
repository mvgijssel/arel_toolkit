...
dir_config('pg', pg_include_dir, pg_lib_dir)

pg_ext = Gem.loaded_specs.fetch('pg')
pg_ext_inlude_dir = File.join(pg_ext.full_gem_path, 'ext')
pg_ext_lib_dir = pg_ext.extension_dir

dir_config('pg_ext', pg_ext_inlude_dir, pg_ext_lib_dir)

if (have_library('pq') || have_library('libpq') || have_library('ms/libpq')) &&

describe PgResultInit do
  ...
  let(:original_result) do
    ActiveRecord::Base
      .connection
      .execute "SELECT 1 AS a, '2' as b, TRUE as c, NULL as d"
  end

  it 'creates a new PG::Result based on another result' do
    columns = [
      { name: 'hello', typid: 0, typlen: 0 },
    ]
  ...
  it 'handles invalid column input' do
    ...
    expect do
      PgResultInit.create(conn, original_result, columns, rows)
  ...
  it 'handles invalid column value member input', aggregate_failures: true do
    ...
    expect do
      PgResultInit.create(conn, original_result, columns, rows)
    end.to raise_error(TypeError, /implicit conversion of Array into String/)

    columns = [{
      name: 'hello',
      typid: 'not an integer',
      typlen: 0,
    }]
    ...
    expect do
      PgResultInit.create(conn, original_result, columns, rows)
    end.to raise_error(TypeError, /implicit conversion of String into Integer/)
  end
  ...
  it 'handles invalid connection input' do
    ...
    expect do
      PgResultInit.create(conn, original_result, columns, rows)
    end.to raise_error(TypeError, /expected kind of PG::Connection/)
  end
  ...
  it 'handles invalid result input' do
    ...
    expect do
      PgResultInit.create(conn, original_result, columns, rows)
    end.to raise_error(TypeError, /expected kind of PG::Result/)
  end

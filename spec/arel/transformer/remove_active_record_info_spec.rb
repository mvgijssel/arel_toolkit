describe Arel::Transformer::RemoveActiveRecordInfo do
  ...
  it 'removes the type_caster stored on the Arel::Table' do
    ...
    arel_table = Arel.enhance(arel).child_at_path(['ast', 'cores', 0, 'source', 'left']).object

    new_arel = Arel::Transformer::RemoveActiveRecordInfo.call(arel, next_middleware)
    new_arel_table =
      Arel.enhance(new_arel).child_at_path(['ast', 'cores', 0, 'source', 'left']).object

    expect(arel_table.instance_values.fetch('type_caster')).to be_a_kind_of(
      ActiveRecord::TypeCaster::Map
    )

    expect(new_arel_table.instance_values.fetch('type_caster')).to be_nil
  end

  def target_children(arel)
    child_path = ['ast', 'cores', 0, 'wheres', 0, 'children']
    Arel.enhance(arel).child_at_path(child_path).object.map(&:right)
  end
  ...
  it 'raises with an unknown BindParam value' do
    ...
    }.to raise_error(/Unknown value cast/)
  end
end

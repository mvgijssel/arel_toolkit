  ...
  module SqlToArel
    class PgQueryVisitor
      ...
      def visit_FuncCall(attribute)
        ...
        func.orders = (attribute.agg_order ? visit(attribute.agg_order) : []) unless func.is_a?(::Array)
        func.filter = (attribute.agg_filter ? visit(attribute.agg_filter) : nil) unless func.is_a?(::Array)
      ...
      def visit_InsertStmt(attribute)
        ...
        insert_statement.values = if attribute.select_stmt.present?
                                    ...
                                    insert_statement.values = select_stmt.values_lists if select_stmt.present?
                                  else
                                    insert_statement.values = Arel::Nodes::DefaultValues.new
                                  end

        insert_statement.returning = visit(attribute.returning_list, :select)
        insert_statement.conflict = visit(attribute.on_conflict_clause) if attribute.on_conflict_clause
        insert_manager
      end
      ...
      def visit_IntoClause(attribute)
        raise "Unknown on_commit `#{attribute.on_commit}`" unless attribute.on_commit == :ONCOMMIT_NOOP
      ...
      def visit_Node(attribute, context = nil)
        ...
        # TODO: See if there is an Arel node for this.
        return '()' if node_key.nil?
      ...
      def visit_SelectStmt(attribute, context = nil)
        ...
        select_core.top = ::Arel::Nodes::Top.new visit(attribute.limit_count) if attribute.limit_count
        ...
        select_statement.lock = visit(attribute.locking_clause) if attribute.locking_clause

        if attribute.values_lists
      ...
      def visit_SQLValueFunction(attribute)
        {
          ...
          SVFOP_CURRENT_TIMESTAMP_N: -> { Arel::Nodes::CurrentTimestamp.new(precision: attribute.typmod) },
          ...
          SVFOP_LOCALTIMESTAMP_N: -> { Arel::Nodes::LocalTimestamp.new(precision: attribute.typmod) },
      ...
      def visit_WindowDef(attribute)
        if attribute.name.present? && attribute.partition_clause.empty? && attribute.order_clause.empty?
          return Arel::Nodes::SqlLiteral.new(attribute.name)
        end

        instance = attribute.name.blank? ? Arel::Nodes::Window.new : Arel::Nodes::NamedWindow.new(attribute.name)
      ...
      def visit_DeallocateStmt(attribute)
        Arel::Nodes::Dealocate.new attribute.name
      end
